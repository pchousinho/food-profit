CREATE OR REPLACE PACKAGE BODY PKG_COMMON_LOG AS
   ----------------------------------------------------------------------------
   -- get_log_level
   ----------------------------------------------------------------------------
   ...
   RETURN BOOLEAN IS
      ...
      CURSOR c_get_log_level IS
         SELECT value
          ...
          WHERE code = pkg_common_gv.g_log_level_parameter;
      --
   BEGIN
      OPEN c_get_log_level;
      FETCH c_get_log_level INTO o_log_level;
      IF c_get_log_level%NOTFOUND
      THEN
   ...
   EXCEPTION
      ...
      THEN
         o_error_message := pkg_common_utils.get_message_text(i_message_key => pkg_common_err.err$log_level_not_found);
         --
         RETURN FALSE;
         --
      WHEN OTHERS
      THEN
         --
         IF c_get_log_level%ISOPEN
         THEN
            CLOSE c_get_log_level;
         END IF;
         --
         o_error_message := pkg_common_utils.get_message_text(i_message_key => pkg_common_err.err$get_log_level);
         --
         RETURN FALSE;
         --
   END get_log_level;
   ...
   PROCEDURE handle_logs(i_log_level       IN logs.log_level%TYPE
                        ...
                        ,i_log_user        IN logs.log_user%TYPE         DEFAULT pkg_common_utils.get_app_user)
   IS
      l_log              logs%rowtype;
      l_custom_exception EXCEPTION;
   BEGIN
      ...
      l_log.code            := i_code;
      l_log.error_message   := pkg_common_utils.get_message_text(
                                  i_message_key => i_code
      ...
      --
      IF NOT pkg_common_log.insert_log_autonomous(i_log => l_log)
      THEN
         RAISE l_custom_exception;
      END IF;
      --
